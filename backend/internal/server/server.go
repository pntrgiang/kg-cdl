			...
			r.Group(func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					r.Patch("/inventory/{id}/status", s.handleUpdateInventoryStatus)
						r.Patch("/inventory/{id}/price", s.handleUpdateInventoryPrice)
						r.Patch("/inventory/{id}/booking", s.handleSetBookingOpen)
						r.Put("/release-override", s.handleSetReleaseOverride)
						r.Post("/manage/promote-release", s.handlePromoteRelease)
						r.Post("/manage/release-pause", s.handleSetReleasePause)
						r.Get("/manage/release-status", s.handleReleaseStatus)
